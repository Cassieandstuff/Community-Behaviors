...
// Stage 4 bindings-resolve pass (ResolveBehaviorBindings + its helpers) was extracted to
// havok-model (havok/model/ResolveBindings.cpp) so the schema compile path can reach it without
// a havok-core dependency. It runs before this builder (which reads pre-resolved indices).
