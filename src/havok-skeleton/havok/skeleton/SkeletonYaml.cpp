...
// Parse the optional per-bone `physics:` block (mass/radius + optional joint).
...
static void emitBoneFields(std::ostream& o, const std::string& ind,
                           const SkeletonBoneData& b, const SkeletonData& d);   // fwd
static void emitBumper(std::ostream& o, const SkeletonData& d);                 // fwd
...
bool EmitSkeletonYamlTree(const SkeletonData& d, const fs::path& dir, std::string* err) {
    ...
    if (ec) { if (err) *err = "cannot create " + (dir / "bones").string() + ": " + ec.message(); return false; }
    {
        ...
        if (!fo) { if (err) *err = "cannot write " + fp.string(); return false; }
        const std::string s = o.str();
        fo.write(s.data(), static_cast<std::streamsize>(s.size()));
    }
    for (const auto& b : d.bones) {
        std::ostringstream o;
        emitBoneFields(o, "", b, d);
...
bool LoadSkeletonLayerFromTexts(const std::vector<std::pair<std::string, std::string>>& boneFiles,
    ...
    if (!bonelistText.empty()) applyBoneOrder(out, parseOrderIndex(bonelistText));
    return true;
}
