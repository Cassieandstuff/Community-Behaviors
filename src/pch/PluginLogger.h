...
//
// Single canonical copy lives here at include/PluginLogger.h (next to PCH.h);
// the plugin-world block of PCH.h pulls it in via __has_include, and the global
// include/ root (added by the root CMakeLists) puts it on every target's path.
// (Formerly duplicated per-plugin under <plugin>/include/PluginLogger.h.)
//
// Usage in Plugin.cpp:
//
//   SKSEPluginLoad(...) {
